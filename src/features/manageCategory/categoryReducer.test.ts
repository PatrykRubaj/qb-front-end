import { CategorySection, EntityStatus, Category } from '../../redux/state';
...
describe('Category reducer', () => {
  it('Should return trimmmed name property when adding new category', () => {
    ...
    const state = categoryReducer(initState, addCategory(newCategory));
  ...
  it('Should return trimmmed name property when saving existing category', () => {
    ...
    const state = categoryReducer(initState, editCategory(changedCategory));
  ...
  it('Should return categories in new order id2, id3, id1', () => {
    ...
    const state = categoryReducer(
      initState,
      moveCategory({ startIndex: 0, endIndex: 2 })
    );
